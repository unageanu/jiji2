...
describe "初期化" do
  ...
  context '認証後' do
    ...
    it 'GET /setting/rmt-broker/available-securities で利用可能な証券会社が取得できる' do
    ...
    it 'GET /setting/rmt-broker/available-securities/$id/configuration_definitions で証券会社の設定値が取得できる' do
      r = @client.get('/setting/rmt-broker/available-securities/mock/configuration_definitions')
      expect(r.status).to eq 200
      p r.body
    end

    it 'GET/PUT /setting/rmt-broker/available-securities/$id/configurations で証券会社の設定値を取得/変更できる' do
      r = @client.get('/setting/rmt-broker/available-securities/mock/configurations')
      expect(r.status).to eq 200
      p r.body
    end
