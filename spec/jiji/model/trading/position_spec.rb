...
describe Jiji::Model::Trading::Position do
  ...
  it "バックテスト向け設定でPositionを作成できる" do
    ...
    position = Jiji::Model::Trading::Position.create(
        "test", nil, 1, 1, 10000, :buy, @data_builder.new_tick(1))
    ...
    expect(position.trading_unit).to eq(10000)
    ...
    position = Jiji::Model::Trading::Position.create(
        "test", nil, 2, 2, 10000, :sell, @data_builder.new_tick(1))
  ...
  it "RMT向け設定でPositionを作成できる" do
    ...
    position = Jiji::Model::Trading::Position.create(
        nil, "1", 2, 100, 10000, :sell, @data_builder.new_tick(2))
  ...
  it "update で現在価値を更新できる" do
    ...
    expect(position.profit_or_loss).to eq(19970)
    
    
    position = Jiji::Model::Trading::Position.create(
        nil, 1, 1, 10, 10000, :sell, @data_builder.new_tick(1))
  ...
  it "close で約定済み状態にできる" do
    ...
    position = Jiji::Model::Trading::Position.create(
        nil, "1", 2, 1, 10000, :buy, @data_builder.new_tick(1))
    ...
    position = Jiji::Model::Trading::Position.create(
        "test", nil, 1, 1, 10000, :sell, @data_builder.new_tick(1))
